...
def main() -> None:
    ...
    if HAS_TENSORBOARD:
        ...
        writer.add_text("config", str(model_cfg), 0)
        print(f"[tb] logs at {tb_dir}", flush=True)
    ...
    for it in range(cfg.max_iters):
        ...
        if it % cfg.eval_interval == 0 or it == cfg.max_iters - 1:
            ...
            if writer is not None:
                writer.add_scalar("eval/train_loss", train_metrics["train"], it)
                writer.add_scalar("eval/dev_ppl", dev_ppl, it)
    ...
    if writer is not None:
        writer.add_scalar("eval/best_dev_ppl", best_dev_ppl, cfg.max_iters)
        writer.close()
